function [A,B,Z,T] = duffing_linearization(x_l,u_l,dt,c,m,k1,k3,box)
...
nSamples = 10000;
samples = unifrnd(box(1),box(2),[2 nSamples]);
T = 1.8743;

% wb = waitbar(0,'Lipschitz sampling started ...');
% for i = 1 : nSamples
%     Phi_eval = double(subs(Phi_lip_fun,x,samples(:,i)));
%     if Phi_eval > T
%         T = Phi_eval;
%     end
%     waitbar(i/nSamples);
