function create_gif(filename,targetDir,x,dt,c,m,k1,k3,skip)
...
% animation loop to create .png files
wb = waitbar(0,'image generation in progress ...');
for frame = 1:length(positions)
    ...
    % export .png
    exportgraphics(gcf, strcat('animation/frame',num2str(frame),'.png'),'Resolution', 500);
    waitbar(frame/length(positions));
end
close(wb);

% actual gif generation
files = dir(fullfile(targetDir, '*.*'));
num_files = sum(~[files.isdir])-1;

wb = waitbar(0,'writing GIF ...');
for idx = 1:num_files
    [RGB,~] = imread(strcat(targetDir,'frame',num2str(idx),'.png'));
